...
export default function UserManagementPage() {
  ...
  // ✅ FE-NEW-07: State untuk menampilkan jumlah PENDING di header
  const [pendingCount, setPendingCount] = useState(0);

  // 🔥 DIPISAH AGAR BISA DIPANGGIL ULANG
  async function loadUsers() {
    try {
      ...
      // ✅ FE-NEW-07: Hitung ulang pending count setiap load
  ...
  // 🔥 AUTO REFRESH DARI TELEGRAM APPROVAL
  useEffect(() => {
    socket.emit("join_admin_room");

    socket.on("admin_refresh", async (payload) => {
      console.log("🔄 Admin auto refresh triggered", payload);

      try {
        await loadUsers();

        toast.success("Data user diperbarui otomatis");
      } catch (err) {
        console.error(err);
      }
    });

    // ✅ FE-NEW-07: Live Toast saat ada guru baru mendaftar
    socket.on("new_teacher_registered", async (payload: { name?: string }) => {
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            <h1 className="text-3xl font-black text-slate-800 flex items-center gap-3">
              User Management 👥
              {/* ✅ FE-NEW-07: Badge PENDING di header */}
              {pendingCount > 0 && (
                <span className="bg-amber-100 text-amber-700 border border-amber-200 text-xs font-black px-3 py-1 rounded-full animate-pulse">
            ...
            <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mt-1">
              Kelola Approval Guru & Hak Akses
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center w-full md:w-auto">
            ...
            <input
              ...
              className="bg-slate-50 border border-slate-200 px-4 py-2.5 rounded-2xl text-sm font-bold text-slate-700 outline-none focus:border-indigo-300 transition-colors w-full sm:w-52"
            />
                    ...
                    <td className="py-5 px-8 text-right">
                      <div className="flex justify-end gap-2">
                          ...
                          user.approvalStatus === "PENDING" && (
                            <>
                              <button
                                ...
                                className="p-2.5 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 shadow-lg shadow-emerald-100 transition-all"
                              >
                                ✅
                              </button>
                              ...
                              <button
                                ...
                                className="p-2.5 bg-rose-500 text-white rounded-xl hover:bg-rose-600 shadow-lg shadow-rose-100 transition-all"
                              >
                                ❌
                              </button>
                            </>
                        ...
                        <button
                          ...
                          className="px-4 py-2 text-rose-500 font-black text-[10px] hover:bg-rose-50 rounded-xl transition-all"
                        >
                          Hapus
                        </button>
                      </div>
