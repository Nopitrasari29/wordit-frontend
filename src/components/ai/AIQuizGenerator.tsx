...
export default function AIQuizGenerator({ level, template, onFinish }: AIProps) {
    ...
    async function generate() {
        if (topic.length < 3) return toast.error("Topik kependekan, Bro!");
        setLoading(true);
        try {
            const res = await fetch("http://localhost:3000/api/ai/generate-quiz", {
                ...
                body: JSON.stringify({
                    topic,
                    educationLevel: "SD", // Paksa SD
                    templateType: template,
                    // Tambahkan instruksi gaya bahasa biar gak kaku kayak robot
                    style: "kids",
                    customPrompt: "Gunakan bahasa anak SD kelas 1-3 yang sangat sederhana. Hindari kata sulit seperti kajian, struktur, atau komponen."
                }),
            });
            ...
            if (json.success) {
                setResult(json.data);
                toast.success("Magic Berhasil! Soalnya ramah anak nih. ✨");
            } else {
                toast.error(json.message || "AI-nya lagi ngambek.");
            }
        } catch (err) {
            toast.error("Gagal konek ke server AI.");
        } finally {
            setLoading(false);
        }
    }
            ...
            <div className="relative z-10">
                <div className="mb-10">
                    ...
                    <p className="text-slate-400 font-bold text-sm">
                        AI akan merancang kuis dengan bahasa yang mudah dimengerti anak SD.
                    </p>
                </div>

                <div className="flex flex-col md:flex-row gap-4 mb-10">
                    <input
                        placeholder="Ketik topik kuis (Contoh: Pelajaran Berhitung)..."
                        value={topic}
                    ...
                    >
                        {loading ? "Lagi Mikir..." : "Generate Magic 🪄"}
                    </button>
                </div>
                    ...
                    <div className="space-y-6 pt-10 border-t border-white/10 animate-in fade-in slide-in-from-top-4 duration-500">
                        <div className="flex justify-between items-center">
                            <p className="text-indigo-400 font-black text-xs uppercase tracking-widest">
                                Preview ({result.words.length} Soal)
                            </p>
                            <button
                                onClick={() => {
                                    onFinish(result.words);
                                    setResult(null);
                                    setTopic("");
                                }}
                        ...
                        <div className="grid grid-cols-1 gap-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
                            {result.words.map((q: any, i: number) => (
